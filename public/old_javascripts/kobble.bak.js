...
window.addEvent('domready', function(){
  console.profile()
  intf = new Interface();
  intf.activate();        // sometimes it needs to refer to itself.
  console.profileEnd()    //Profile (65.38ms, 7517 calls)
});
...
var Interface = new Class({
  ...
  startDragging: function (helper) {
    ...
    this.debug('catchers will be: ', 3);
    this.debug(catchers, 3);
    return catchers;
  },
  stopDragging: function (helper) {
    this.debug('stopdragging: ', 2);
...
var TabSet = new Class({
  initialize: function(tag){
    this.tabs = [];
    this.tag = tag;
    this.headcontainer = $('headbox_' + this.tag);
    this.container = $('box_' + this.tag);
...
var ScratchSet = new Class({
  ...
  initialize: function(tag){
    this.parent(tag);
    this.container = $('scratchpad');
    this.tabscontainer = $('scratchpad_tabs');
    this.pagescontainer = $('scratchpad_pages');
    this.isopen = false;
